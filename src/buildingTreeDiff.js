const buildingTreeDiff = (object1, object2) => {
  ...
  Object.keys(object1).forEach((key) => {
    ...
    && object1[key] !== object2[key]) {
      typeof (object1[key]) === 'object'
        ? result.push(`- ${key}: `, buildingTreeDiff(object1[key], object1[key])) : result.push(`- ${key}: ${object1[key]}`);
      typeof (object2[key]) === 'object'
        ? result.push(`+ ${key}: `, buildingTreeDiff(object2[key], object2[key])) : result.push(`+ ${key}: ${object2[key]}`);
    } else {
      typeof (object1[key]) === 'object'
        ? result.push(`- ${key}: `, buildingTreeDiff(object1[key], object1[key])) : result.push(`- ${key}: ${object1[key]}`);
    }
